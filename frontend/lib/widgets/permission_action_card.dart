  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
          color: action.requiresConfirmation ? riskColor.withOpacity(0.5) : (isDark ? AppTheme.darkBorder : const Color(0xFFE5E7EB)),
          width: 1,
        ),
      ),
          ...
          Row(
            children: [
              Container(
                ...
                child: Icon(_getActionIcon(action.type), size: 18, color: riskColor),
              ),
              ...
              children: [
                OutlinedButton(
                  ...
                  style: OutlinedButton.styleFrom(
                    foregroundColor: isDark ? AppTheme.darkTextSecondary : Colors.black54,
                ...
                ElevatedButton.icon(
                  ...
                  label: Text(_isLoading ? 'Executing...' : 'Approve & Run', style: const TextStyle(fontSize: 12.5, fontWeight: FontWeight.w600)),
                ),
              ],
            ),
          ] else ...[
              ...
              children: [
                Icon(
                  action.status == ActionStatus.completed
                      ...
                      : (action.status == ActionStatus.declined ? Icons.cancel_rounded : Icons.pending_outlined),
                  size: 14,
                  color: action.status == ActionStatus.completed
                      ? AppTheme.brandAccent
                      : (action.status == ActionStatus.declined ? AppTheme.dangerRed : Colors.grey),
                ),
                const SizedBox(width: 5),
                Text(
                  action.status == ActionStatus.completed
                      ? 'Action successfully executed'
                      : (action.status == ActionStatus.declined ? 'Action declined by user' : 'Processing...'),
                  style: TextStyle(
                    fontSize: 11.5,

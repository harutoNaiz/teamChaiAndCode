...
class AgentService {
  ...
  String backendBaseUrl = 'http://10.0.2.2:5000'; // Default Android emulator host loopback
  ...
  Future<ChatMessage> sendMessage({
    ...
    if (backendMode == AgentBackendMode.flaskBackend) {
      try {
        ...
        debugPrint('Flask backend connection failed: $e. Falling back to simulation mode.');
      }
    }
  ...
  Future<ChatMessage> _simulateAgentResponse(
    ...
    if (lower.contains('offer letter') || lower.contains('internship') || lower.contains('pdf') || lower.contains('find')) {
      ...
      if (lower.contains('whatsapp') || lower.contains('send') || lower.contains('share')) {
        actions.add(AgentAction(
          id: 'act-${Random().nextInt(9999)}',
          type: 'send_whatsapp',
    ...
    } else if (lower.contains('clean') || lower.contains('organize') || lower.contains('download')) {
      thoughtProcess = '1. Scan Download folder for file extensions\n2. Create category buckets (Docs, Media, APKs)\n3. Prepare batch move operation';
      ...
      responseText = 'I scanned your `Downloads/` directory and grouped 24 files:\n\n'
      ...
      responseText = 'I am your smartphone agent powered by **teamChaiAndCode**.\n\n'
  ...
  Future<bool> executeAction(AgentAction action) async {
    ...
    if (action.type == 'send_whatsapp') {
      ...
      action.result = {'sent': true, 'timestamp': DateTime.now().toIso8601String()};
      return true;
    }

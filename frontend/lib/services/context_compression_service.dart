...
class ContextCompressionService {
  static const int recentWindowSize =
      4; // Number of recent messages kept verbatim
  static const int compressionThreshold =
      6; // Trigger compression when total messages exceed this

  static final ContextCompressionService instance =
  ...
  void _compressOlderTurns(ChatSession session) {
    ...
    final olderMessages =
        session.messages.sublist(0, session.messages.length - recentWindowSize);
    ...
    for (final msg in olderMessages) {
      if (msg.isUser) {
        buffer
            .writeln('- User asked/requested: "${_truncate(msg.content, 80)}"');
      } else {
        ...
        buffer.writeln(
            '  • Key Agent response: "${_truncate(msg.content, 100)}"');
      }
    }
